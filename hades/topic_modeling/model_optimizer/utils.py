...
def save_data_for_app(
    ...
    for model_optimizer in model_optimizers:
        filter_name = "_".join([
            value.replace(":", "").replace(" ", "_").replace(",", "").replace("/", "_").replace("(", "").replace(
                ")", "").replace("&", "").replace("-", "_") for value in model_optimizer.column_filter.values()
        ])
        ...
        config_dict['sections'][filter_name] = {
            "probs": path + filter_name + "_probs.csv",
    ...
    if do_summaries and openai.api_key is not None:
        prepare_app_summaries(model_optimizer, n_extract_sentences, path + "summaries.json")
